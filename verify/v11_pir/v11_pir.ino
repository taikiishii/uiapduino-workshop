 ...
 *   ここを実測して、e4「人感センサー」の配線とコードを決める。
 *
 * ★ 2026-09-25 追記：資料で見通しがついた
 *   ・CH32V003 データシート（V1.3・表3-16）の VIH 最小は 0.22*(VDD-2.7)+1.55。
 *     VDD＝5V なら **約 2.06V**。3V はそれより高いので **digitalRead で読めるはず**。
 *   ・SB412A の回路図では、センサー IC は基板の 3.0V レギュレータで動き、
 *     OUT はその IC から直接（20kΩ でGNDへ）出ている。**オープンコレクタではない**。
 *     → 下の USE_PULLUP（5V へ引き上げる）は、3V の出力とぶつかるので**試さない**。
 *   ・ピンは 1＝電源・2＝VOUT・3＝GND。TIME のつまみを上・レンズを右にして、
 *     上のパッドが四角い（＝1番）。実物で確かめる。
 *
 * つなぎかた
