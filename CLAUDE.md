...
- 章フォルダ名の頭 … **i=導入編 / b=基本編 / c=制御編 / e=発展編 / m=拡張編 / p=作品編 / x=付録**
...
- **PWM が使えるピンは 2・5・6・12 のみ**（公式図は 0 も可とするが、
  **実機では 0 は `analogWrite` 無反応**。1・10 と同じ。2026-09-23 実測）
- **`analogWrite` を出したピンは `analogWrite(pin, 0)` で止める。**
  `digitalWrite(pin, LOW)` では**止まらない**（PWM が出っぱなしになる）。
  ⚠ 標準の Arduino なら止まる。**このコアに `turnOffPWM` が無い**ため
