...
使うのは **MCP9700** のような**アナログ式**のセンサーだよ。足は3本。

- 温度が上がると、出てくる**電圧が高くなる**
- 1℃上がるごとに **10mV（0.01V）** ずつ

> ⚠ 平らな面を手前にして、左から **＋・出力・−** の順（品種によってちがう）。
> 逆につなぐと熱くなるので、**必ず確かめてから**電源を入れよう。
...
> ⚠ 向きをまちがえると、センサーが**熱くなる**。
> つないだら、まず指でさわって熱くないか確かめてね。
...
float ondo() {
  ...
  float volt = v * 3.3 / 1024;   // 電圧に直す
  return (volt - 0.5) * 100;     // ℃ に直す
}
```
...
> 数字が見えないので、**しきい値をこえたかどうか**をLEDで表す。
> これがこのボードでの「数を見る」方法だったね。
